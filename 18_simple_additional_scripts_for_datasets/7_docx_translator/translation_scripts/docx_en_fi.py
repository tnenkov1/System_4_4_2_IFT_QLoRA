...
PROMPT_FILE = os.path.join(BASE_DIR, "prompts", "translate_en_fi.txt")  # <-- EN→FI
...
def load_prompt_template():
    if not os.path.exists(PROMPT_FILE):
        default_prompt = "Translate the following text from English to Finnish. Retain professional tone and formatting (bold, italic, underline, headings). Text: {TEXT}"
...
def translate_docx(file_path, session: requests.Session):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{os.path.splitext(file_name)[0]}_fi.docx")
