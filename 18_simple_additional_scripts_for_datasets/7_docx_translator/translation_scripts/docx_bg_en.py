...
PROMPT_FILE = os.path.join(BASE_DIR, "prompts", "translate_bg_en.txt")

MODEL = ""
API_URL = "http://localhost:11434/api/generate"

MAX_WORKERS = 2  # За по-малки VRAM
MAX_RETRIES = 3
SLEEP_BETWEEN_REQUESTS = 0.05  # сек
...
def load_prompt_template():
    if not os.path.exists(PROMPT_FILE):
        default_prompt = "Translate the following text from Bulgarian to English. Retain the professional tone and formatting (bold, italic, underline, headings). Text: {TEXT}"
...
def translate_docx(file_path, session: requests.Session):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{os.path.splitext(file_name)[0]}_en.docx")
    tmp_path = out_path + ".tmp"

    # 1. Събираме параграфи и клетки
    targets = []
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in tqdm(as_completed(future_to_obj), total=len(future_to_obj), desc=f"Translating {file_name[:20]}"):
            ...
            try:
                ...
                # Запазване на оригиналното форматиране (Runs)
                if hasattr(obj, "paragraph_format"):  # Paragraph
                    # Изчистваме параграфа и добавяме нови Runs
                    runs = list(obj.runs)
                    for r in runs:
                        r.clear()
                    # Създаваме нов Run със същото форматиране
