...
PROMPT_FILE = os.path.join(BASE_DIR, "prompts", "translate_en_bg.txt")  # <-- EN→BG

MODEL = ""
API_URL = "http://localhost:11434/api/generate"

MAX_WORKERS = 1  # За по-малки VRAM
MAX_RETRIES = 3
...
def load_prompt_template():
    if not os.path.exists(PROMPT_FILE):
        default_prompt = "Translate the following text from English to Bulgarian. Retain the professional tone and formatting (bold, italic, underline, headings). Text: {TEXT}"
...
def translate_docx(file_path, session: requests.Session):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{os.path.splitext(file_name)[0]}_bg.docx")
    tmp_path = out_path + ".tmp"

    # 1. Collect paragraphs and table cells
    targets = []
